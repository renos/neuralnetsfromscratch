...
class Net(nn.Module):
    def __init__(self):
        ...
        self.m2 = nn.MaxPool2d(kernel_size=(2, 2), stride=2)
    ...
    def forward(self, x):
        x = self.m1(F.relu(self.c1(x)))
        x = self.m2(F.relu(self.c2(x)))
        x = F.relu(self.c3(x))
        x = x.view(x.shape, -1)
        x = F.relu(self.f1(x))
        x = self.f2(x)
...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='LeNetMnist')
